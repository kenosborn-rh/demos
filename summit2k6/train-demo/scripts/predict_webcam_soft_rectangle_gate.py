...
# --- Bright paper gate ---
USE_PAPER_GATE = True
MIN_BRIGHT_FRAC = 0.18      # at least 18% of center box must be bright
MAX_BRIGHT_FRAC = 0.92      # if nearly whole box is bright, probably not a placard
MIN_CENTER_BRIGHT = 0.12    # center region should also be bright enough
BRIGHT_THRESH = 185         # grayscale threshold for "paper-like" brightness
...
def main():
    ...
    while True:
        ...
        if SHOW_WINDOW:
            ...
            if DRAW_CROP_GUIDE:
                cv2.rectangle(overlay, (gx, gy), (gx + gside, gy + gside), box_color, 3)

            # Show paper gate status with a subtle inner outline
            if USE_PAPER_GATE:
            ...
            if SHOW_DEBUG_ON_SCREEN:
                debug_text = (
                    ...
                    f"margin {margin:.3f} | streak {streak} | "
                    f"paper {'yes' if paper_found else 'no'} | "
                    f"bright {bright_frac:.3f}/{center_bright_frac:.3f} | fps {fps_val:.1f}"
                )
                draw_text_with_bg(
                    overlay,
                    debug_text,
            ...
            # Optional mini preview of the bright mask in top-right corner
            if USE_PAPER_GATE and paper_mask is not None:
                mask_small = cv2.resize(paper_mask, (160, 160), interpolation=cv2.INTER_NEAREST)
                mask_small = cv2.cvtColor(mask_small, cv2.COLOR_GRAY2BGR)
                oy, ox = 24, overlay.shape[1] - 184
                overlay[oy:oy + 160, ox:ox + 160] = mask_small
                cv2.rectangle(overlay, (ox, oy), (ox + 160, oy + 160), (255, 255, 255), 1)
                draw_text_with_bg(
                    overlay,
                    "paper gate",
                    (ox + 8, oy + 24),
